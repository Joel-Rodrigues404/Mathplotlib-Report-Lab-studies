...
plt.plot(comprimento, T2, 'o-', label='Dados experimentais')
plt.xlabel("Comprimento L (m)")
plt.ylabel("Período T² (s²)")
plt.title("Comprimento vs Período ao quadrado")
plt.grid(True)
plt.legend()
plt.show()

# Ajuste linear para determinar inclinação
coef = np.polyfit(comprimento, T2, 1)
inclinação = coef[0]
print("Inclinação da curva T² x L:", inclinação)
